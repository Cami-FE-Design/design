...
| **DW3.4** | P2 | **Built — `/catalogs/deals`, a real route.** The list carries a Locations column that tells a chain-wide offer from a local one, a dialog that changes it, and a refusal to save a deal with nothing chosen. The rule is in `lib/locations/promotion-scope.ts`. `lib/locations/promotion-scope.ts` states where a promotion runs — a named chain-wide case or a list, and an empty list refused rather than standing in for "everywhere". The dev repo's `promotion-discount-ui` already carries the axis (`allVenues` + `venues`) but its mapper turns `allVenues: true` into `locationIds: []` — so empty means *all* there, while R24 here says an empty scope never resolves to all. Multi-location had not started when that was written, which is why it reads that way; `fromAvailability()` converts without inheriting the ambiguity. **Worth raising with Faisal.** |
