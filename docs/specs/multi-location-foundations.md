...
| **DW3.4** | P2 | **Built as the rule, not the feature.** `lib/locations/promotion-scope.ts` states where a promotion runs — a named chain-wide case or a list, and an empty list refused rather than standing in for "everywhere". The promotions UI itself belongs to the dev repo's `promotion-discount-ui`, which already carries the axis (`allVenues` + `venues`) but whose mapper turns `allVenues: true` into `locationIds: []` — so empty means *all* there, while R24 here says an empty scope never resolves to all. Multi-location had not started when that was written, which is why it reads that way; `fromAvailability()` converts without inheriting the ambiguity, and this is the shape to build against. **Worth raising with Faisal.** |
