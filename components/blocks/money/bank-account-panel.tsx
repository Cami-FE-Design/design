...
import { BadgeCheckIcon, ClockIcon, LandmarkIcon, LockIcon, ShieldAlertIcon } from "lucide-react"
...
const VERIFICATION: Record<
  VerificationState,
  { label: string; variant: "secondary" | "warning" | "muted"; icon: typeof BadgeCheckIcon }
> = {
  verified: { label: "Verified", variant: "secondary", icon: BadgeCheckIcon },
      ...
      <div className="flex flex-col gap-6">
        ...
        <section className="flex w-full flex-col gap-6 rounded-2xl border border-border/60 p-5 sm:w-fit sm:min-w-146 sm:max-w-146">
          ...
          <div className="flex flex-col gap-2">
            <h4 className="text-sm font-medium text-foreground">Who pays into it</h4>
            <ul className="flex flex-col gap-2">
              {activeRails.map((rail) => (
                ...
                >
                  ...
                  <span className="text-xs text-muted-foreground">
                    {rail === "online" ? "Online payments" : "Card machine payments"}
                  </span>
                </li>
              ))}
            </ul>
        ...
        <section className="flex w-full flex-col gap-6 rounded-2xl border border-border/60 p-5 sm:w-fit sm:min-w-146 sm:max-w-146">
          <header className="flex flex-col gap-1">
            ...
            <p className="text-sm leading-5 text-muted-foreground">
              Every time this account was changed, or someone tried to. Permanent, and it cannot be
              edited or removed.
            </p>
          </header>
          ...
          <ul className="flex flex-col divide-y divide-border/60">
            {history.map((change) => (
              <li key={change.id} className="flex items-start justify-between gap-4 py-3">
                ...
                <Badge
                  variant={change.outcome === "failed" ? "warning" : "secondary"}
                  className="shrink-0"
                >
                  {change.outcome === "failed" ? "Did not apply" : "Applied"}
                </Badge>
              </li>
            ))}
          </ul>
