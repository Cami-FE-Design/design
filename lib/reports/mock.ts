...
export const MOCK_GROUPED: Record<string, Record<string, ReportRow[]>> = {
  "sales-summary": {
    ...
    Location: [
      ...
      {
        // Took nothing this period. Kept as a row rather than dropped: "no
        // sales there" and "missing from this report" are different answers,
        // and only one of them is worth acting on (SCR-15's own rule).
        type: "Shampooch Dubai Marina",
        salesQty: 0,
        itemsSold: 0,
        grossSales: 0,
        totalDiscounts: 0,
  ...
  "appointments-summary": {
    Location: [
      ...
      },
      {
        ...
        pctOnline: 38,
        pctCancelled: 9,
        // The row worth finding: nearly one booking in five never turns up,
        // which no business-wide average would have shown.
        pctNoShow: 18,
        totalClients: 35,
        newClients: 6,
        pctNewClients: 17,
        pctReturningClients: 83,
      },
      {
        location: "Shampooch Downtown Dubai",
        appointments: 28,
        services: 31,
        pctRequested: 21,
        totalApptValue: 6100,
        ...
        newClients: 15,
        pctNewClients: 58,
        pctReturningClients: 42,
      },
      {
        location: "Shampooch Al Majaz",
        appointments: 12,
        services: 14,
        pctRequested: 33,
        totalApptValue: 1900,
        ...
        newClients: 4,
        pctNewClients: 36,
        pctReturningClients: 64,
      },
    ],
    // Team member and Service overlap: an appointment with two groomers or
    // two services sits under each, so these rows add up to more than the 145
    // appointments at the branches (the Total row reads the distinct figure).
    // Services and value are per service line and still add up exactly.
    "Team member": [
      {
    ...
    Service: [
      ...
      },
      {
        ...
        newClients: 9,
        pctNewClients: 22,
        pctReturningClients: 78,
      },
      {
        location: "Teeth Cleaning",
        appointments: 20,
        services: 20,
        pctRequested: 15,
        totalApptValue: 3370,
  ...
  // Discount summary re-slices one set of discounted sales three ways, so the
  // totals agree whichever pill is picked (Σ AED 380 off AED 5,100, the Jul
  // "Discounts" line on Finance summary). Names and types are the deals in
  // lib/deals/mock.ts; a sale counts once per deal applied (Times used).
  "discount-summary": {
    "Discount name": [
      {
        discount: "Summer groom offer",
        timesUsed: 6,
        itemsDiscounted: 7,
        grossSales: 1500,
        itemDiscounts: 120,
...
/**
 * The Total row for a dimension whose rows overlap (GroupBy.overlapNote): the
 * whole report counted once, where summing the rows would count an appointment
 * under each of its services. Only the columns that overlap are stored; the
 * rest still sum. Matches the Location slice, where every row is disjoint.
 */
export const MOCK_DISTINCT_TOTALS: Record<string, ReportRow> = {
  "appointments-summary": { appointments: 145, totalClients: 130, newClients: 39 },
}
...
export const MOCK_ROWS: Record<string, ReportRow[]> = {
  ...
  "sales-log-detail": [
    ...
    },
    {
      ...
      saleNo: "14",
      location: "Shampooch Dubai Marina",
      type: "Service",
      item: "Nail Trim",
      category: "Grooming",
