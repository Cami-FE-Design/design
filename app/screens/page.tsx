...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    screens: [
      ...
      {
        path: "/playground#multi-location-branch-roster",
        label: "Branch roster · SCR-10 (playground)",
        note: "R05, DW2.3 and DW2.4, which are complementary rather than the alternatives an earlier version of the spec had them as — that framing was the only thing blocking this screen. A roster belongs to one branch, and somebody assigned to two appears on both with the hours they work there, because DW2.3’s acceptance is that booking offers them at a branch only during those hours. Lena is JVC mornings and Jumeirah evenings: two branches in one day, no conflict. Sara is rostered at both over the same hours, which is the one clash DW2.4 names. Mariam has two overlapping shifts at JVC and is deliberately not flagged — ADR-023 allows overlap inside a branch, and marking it would report the product’s own behaviour as an error. Nothing blocks: whether booking refuses a cross-branch overlap extends ADR-023, which the PRD marks 🔴, so the roster states the fact and leaves the rule to engineering.",
      },
      {
  ...
  },
  {
    ...
    screens: [
      ...
      },
      {
        ...
        note: "The business-wide answer to 'did today's reminders actually go out?', which the per-appointment timeline cannot give. Grouped by day with a per-day count; channel and status filters over every send (recipient, time, body, cost); failure reasons inline, with two in the demo data — a landline that cannot take SMS and a hard-bounced mailbox. See docs/specs/notifications-sender-id-and-rates.md.",
      },
      {
        path: "/sales/appointments-list?ref=b-002&view=activity",
        label: "Notification events in appointment activity",
        note: "Opens the detail sheet on Tom Cassidy / Luna straight onto the activity panel (&view=activity; also the ⋮ in the sheet footer). Sends are events in the existing timeline — channel icon on the dot, status pill, recipient in the meta line — and a failed send is the only event that tints. Swap the ?ref= for b-003 (queued 2h reminder), b-004 (failed SMS) or b-005 (hard bounce), keeping &view=activity. See docs/specs/notifications-sender-id-and-rates.md.",
      },
      {
      ...
      },
      {
        ...
        note: "Opens straight into one editor (&ce=<event>:<channel>). Form left, sticky live preview right, stacking below lg. Subject and body are plain text with clickable {{placeholder}} chips, and a chip already used in the body tints. Type a nonsense token like {{discount}} — it sends as written rather than being blanked. Save is disabled until something changes; 'Reset to default' appears only once a template is customised. See docs/specs/DSG-83-communication-templates.md.",
      },
      {
