...
/**
 * Which audience a group belongs to. 31 groups in one flat scroll gave no way
 * to find anything — the lanes are the map's legend, and the index at the top
 * is built from them.
 */
type Lane = "public" | "onboarding" | "business" | "settings" | "hq" | "reference"

const LANES: Array<{ id: Lane; label: string; blurb: string }> = [
  {
    id: "public",
    label: "Public & pet parent",
    blurb: "Everything a pet parent can reach without an account.",
  },
  {
    id: "onboarding",
    label: "Getting in",
    blurb: "Sign-in, invites, and the first-run wizard.",
  },
  {
    id: "business",
    label: "Business app",
    blurb: "The operator's day: booking, selling, catalog, reporting.",
  },
  {
    id: "settings",
    label: "Business settings",
    blurb: "Everything behind the settings dialog.",
  },
  { id: "hq", label: "Cami HQ", blurb: "Our own control plane over Partners." },
  {
    id: "reference",
    label: "Reference",
    blurb: "Component states and demo surfaces, not product screens.",
  },
]
...
const REPORT_HINTS: Record<string, string> = {
  ...
  "performance-dashboard":
    "REVAMPED (DSG-79) — 21 widget cards across Sales & revenue and Appointments & CRM, in Maaz's draft order. Hero KPI strip jumps to any widget — five metrics whose cards sit far down the page; Total sales is not among them, since its card is the very next thing you see. One sticky control bar (date range / filters / Customise) inside the shared report frame; Back to top appears in it after 600px. Customise toggles any widget off and previews the dashboard as Owner / Manager / Staff, which is how the per-report access levels are shown instead of a chip on every card. Add ?state=loading, ?state=empty or ?state=sparse to see the per-shape skeletons and the empty states. New chart primitives: donut (stacked + wide), funnel, ranked bars, capacity heatmap — see /playground for them in isolation.",
  "performance-summary":
...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    screens: [
      ...
      {
        path: "/sign-in/reset-password?token=demo",
        label: "Set a new password",
        note: "Reached from the reset link in the email. Token drives the state — ?token=expired shows the dead-link message instead of the form.",
      },
      {
        path: "/sign-in/accept-invite?token=demo",
        label: "Accept a team invite",
        note: "A team member's first sign-in: they set their own password against the invite token rather than being handed one.",
      },
      {
        path: "/sign-in/welcome",
        label: "Welcome, account ready",
        note: "Terminal screen after sign-up or an accepted invite, before the first-time setup wizard picks up.",
      },
    ],
  },
  {
    ...
    screens: [
      {
        ...
        label: "Resource calendar · Day",
        note: "Bays, rooms and vans as columns instead of staff — the same day grid read by where the work happens rather than who does it. Rows are the resources from lib/scheduling-mock (Grooming Bay 1/2, Bath station, Van), each with its own capacity and the services it can take; blocks are tinted per resource. Prev/next day in the header.",
      },
      {
        path: "/appointments",
        label: "Toolbar + sheets",
        note: "Header toolbar (date, Day/Week, filters, New booking) over a demo body. PRD-143: a booked combo shows up as one line per component service, each led by the layers glyph and prefixed 'Combo - Service' with the pre-discount price struck through, matching the as-built calendar — see Luke Tan / Rocky (b-004), whose Nail clipping and Wash & Blow Dry LG lines both come from 'Wash & Nails Combo', on the hover card and in the detail sheet's Services section. The people grid itself is not mounted on this route yet — it renders in /playground under 'Appointments — toolbar and people grid', which is also the only place the booking blocks and their hover card can be exercised. Clicking a block there now opens <AppointmentDetailSheet>: PRO-68 shipped a hover card plus a second 380px click popover that had no route to the sheet at all, so the calendar could never reach an appointment's details. The click card is gone and the two-artefact model of the as-built app is in place — hover reads, click opens the drawer.",
      },
      {
        path: "/appointments",
        label: "New booking sheet · Pet Address + Pet notes",
        note: "'New booking' → scroll to the Pet Address and Pet notes sections. The Pet Address tick is off by default; ticking it reveals the address block, which reuses the selected client's saved address (Karen Dougall and Maaz Test You have one on file; Aaliyah Hazari doesn't, which forces the manual input; with no client picked yet you get a 'pick a client' hint). DZ-209: selecting a client with notes on file (Karen Dougall has four, Maaz Test You and Aaesha Al Ali have one each) reveals a Client notes card under the client row — the packages-left / 'always asks for Aya' context that changes what gets booked. Aaliyah Hazari has none, so nothing renders. PRD-144: the address field is now the same map search as the billing address, not a plain text box — pick a suggestion (try 'Al Ghozlan', 'Indigo', 'Bay Square') and the line beneath says 'Pinned on the map'; type over it and the pin is dropped, which the line says too. Maaz Test You and Aaesha Al Ali have pinned profile addresses; Karen Dougall's is typed, so reusing hers reports it as un-pinned. Pet notes use the same six tappable categories as the public flow, so notes captured by staff and by the parent stay the same shape. NOTE: this route does not render the calendar grid — the booking blocks and their popover live on /playground.",
      },
      {
        path: "/playground",
        label: "Client notes / Staff Alert (playground)",
        note: "DZ-209, under 'Client notes (Staff Alert)'. Four states: the full 2-of-4 preview with per-note attribution (Karen Dougall), a single unattributed note (Tom Cassidy), the compact 1-line popover rendering, and a client with none — which renders nothing at all rather than an empty card. Mirrors the as-built ClientNoteBanner in cami-business, compact mode included. The hover cards in 'Appointments — pickup & pet notes' were aligned to the as-built EventDetailPopup in cami-business: one card instead of two, identity once (no second pet card), pet address read with the client instead of at the foot of the panel, every service listed with its own performer / duration / price / duration-modifier pills / membership chip, and a service-count-plus-total footer. Everything that changes the appointment moved to the sheet. The last row there is a three-service booking by two groomers with one item on a membership. The DZ-209 card marker is under 'Appointments — toolbar and people grid': booking blocks with either note kind now carry a document glyph in the icon row, so reception can see which appointments have a note without hovering every one.",
      },
      {
        path: "/playground",
  ...
  {
    lane: "business",
    title: "Pet Business, catalogs — Service menu, Categories & Packages (PRO-catalog)",
    description:
      "Service catalog prototype ported from the developer app, backed by local mock state (no API, no permissions). Adding a category/service on one screen is reflected on the other — both share one in-memory store via the catalogs provider at the root layout, which is also what lets a combo created here turn up in the appointment and POS pickers. Prices render in AED.",
    screens: [
      {
  ...
  },
  {
    ...
    screens: [
      ...
      },
      {
        ...
        note: "Same sheet, showing the two new sections between Services and the sale total: a Pickup card (car chip + collection address, only when the booking needs pickup) and a Pet notes card. The sheet also gained the two sections the as-built drawer has and ours did not: a Payment policy card ('Requires confirmation · AED 24.50 deposit requested', read live from the business's payment-policy store, so it disappears when no deposit policy is configured) and a single Checkout as the footer's primary action in place of the old 'Pay now' + 'Complete now' pair. The unsigned-agreement banner moved here from the deleted click popover, where dismissing it only lasted until the pointer left the block. The Services section now lists every service on the booking — b-004 has three by two groomers, one with +10min processing / +10min blocked pills and one struck through as Included in membership — and the Sale total sums those rows rather than the booking's headline price, which used to be only the first item. DZ-209 adds two more: a Client notes card inside the Client section (b-001 to b-004 all have notes; b-003 is Karen Dougall with four, so the '+2 more on the client profile' line shows), and an Appointment note section that this sheet never rendered before — booking.notes existed on the data and only ever surfaced on the calendar popover; it is the last section in the body, after the sale total. Both note blocks follow one rule now: the glyph is the same on every surface because it identifies the card, and only its position changes — leading inside a sheet card (like the pin on Your Pet Address), trailing on the calendar hover card. Client notes sits under its own h2 here rather than carrying an internal label. PRD-144 adds a Navigate button to the Pickup card: b-004's address was picked from the map search so it routes straight to the stored coordinates, while b-002's was typed and can only offer 'Search in Maps' — the same pair shows on the calendar popover in /playground. The sheet opens from the Ref # link in the table — not the row body — so use this deep link. b-004 also has both; b-001 has neither, which is the empty case.",
      },
      {
  ...
  {
    lane: "settings",
    title: "Pet Business, notifications and templates",
    description:
      "Two panels, one story: Notifications decides whether a message sends, Communication templates (DSG-83, the last four links) decides what it says. Sender ID, reminder channels, and per-message usage — Business Settings > Notifications (spec: docs/specs/notifications-sender-id-and-rates.md). Follow-on to the Notifications & Reminders sign-off, which excluded all of this. The Sender ID is one state machine, not three timelines: not-submitted → submitted → approved | rejected, with CAMI as the fallback in three of the four states, so 'just enable SMS in UAE' is satisfied by not-submitted working properly rather than an interim screen. Merchant intent and the Cami HQ channel grant are separate fields and never merged — an ungranted channel locks its column instead of hiding it, and re-enabling at HQ restores exactly what the merchant had. Message cost is stamped per send rather than derived from the current rate, so an HQ rate change can't retroactively rewrite last month's consumption. Walk the Sender ID states and the WhatsApp grant with the two faint demo controls bottom-right — both stand in for decisions taken in Cami HQ, which this portal can't reach.",
    screens: [
      {
        path: "/shell-demo?settings=notifications",
        label: "Notifications, settings",
        note: "Three w-146 cards. Sender ID answers one question — which name will customers see — and now answers it once: the field carries the registered value, and the line under it ('Customers currently see CAMI') renders only when the two differ, which is exactly the submitted and rejected states. In not-submitted and approved the field already says it, so it stays quiet; the notice was trimmed for the same reason, since 'Messages send as CAMI' beside a field reading CAMI (default) was the third telling of one fact. Edit is disabled while a registration is pending, so a merchant can't change the value under a live submission. Reminders: 7 events × 3 channels with the read-only per-message rate under each channel header, and the event labels are links into their Communication templates editor (underline on hover only — permanently underlining all seven turned the table into a list of links). Narrow past sm: the matrix stacks into per-event blocks with labelled switches, rates collapse to one summary line, and it is one tree rather than two copies (the channel wrapper is sm:contents), not a horizontal scroller, since mobile is the prioritised device. Usage this month leads with a share-of-cost bar: SMS is 81% of AED 42.92 on fewer sends than email, which is the question a merchant brings to a usage card and which three right-aligned figures made you do arithmetic to answer. It is a meter, not a chart — the same geometry as the import progress bar, stacked — and it hides with the rates, since a picture of the costs would leak the ratio the figures withhold. Under the total sits a straight-line month-end estimate, suppressed for the first two days of a month where one day extrapolated over thirty-one is noise. The period label derives from the data rather than being a hardcoded '1 – 10 August' that went stale the day after it was written. Totals come from a period aggregate, not from summing the log — a real month is hundreds of sends and the log is paginated, so summing loaded rows under-reported the invoice (AED 0.49 for a month that cost AED 42.92). The demo numbers match Shampooch in the HQ mocks, so this total and that partner's row on /admin/billing agree to the fils. Three faint demo controls bottom-right. Two stand in for decisions taken in Cami HQ (the WhatsApp grant, the four Sender ID states); the third exists purely for coverage — the shipped default has WhatsApp at zero cost, which made the share bar's three-segment state unreachable, so 'Demo: WhatsApp unused / consuming' walks it. With WhatsApp consuming the split reads Email 13% · SMS 56% · WhatsApp 31% of AED 62.18. That consumption is deliberately not mirrored into the log: periodUsage is a period aggregate and the log is the most recent sends, so the two are never derived from each other.",
      },
      {
        path: "/shell-demo?settings=notifications&nt=sender",
        label: "Notifications, Sender ID dialog",
        note: "Compact dialog, not a FullScreenTakeover — a one-field form in a takeover reads as a rendering bug (same call as the terminal rename dialog). Validates on submit rather than per keystroke: 3–11 alphanumeric, and CAMI itself is reserved because it's already the fallback. Saving lands on 'submitted', never 'approved' — registration is an HQ decision. 'Use CAMI instead' is the route back for a merchant who registered the wrong name.",
      },
      {
        path: "/shell-demo?settings=notifications&nr=hidden",
        label: "Notifications, prices withheld",
        note: "The same panel with MERCHANT_SEES_RATES off — an open commercial question made reviewable instead of described. Rates are negotiated per merchant (Pawhaus runs AED 0.14 against a global AED 0.12), so if Cami doesn't want merchants comparing prices this is the shape they get: counts and failures, no per-message rate, no per-channel cost, no per-send cost in the log. Per-channel cost hides with the rates because 412 sends for AED 8.24 is the rate with one division. The period total stays — it's what they owe, it's on the invoice regardless, and one number can't be solved back into two per-channel rates. Default is rates-visible: nothing in the thread forbids showing a merchant their own price, and hiding it is hostile when the invoice reveals it weeks later. One constant in lib/notifications/types.ts flips it.",
      },
      {
        path: "/shell-demo?settings=notifications&nt=log",
        label: "Notifications, log",
        note: "The business-wide answer to 'did today's reminders actually go out?', which the per-appointment timeline structurally can't give. Grouped by day, like the money activity feed, with a per-day count on the header: ungrouped, you had to read every row's time to find the boundary. Rows are one bordered card with divided rows rather than a card each — per-row borders cost a frame, a radius and 16px of padding to separate what a 1px rule already separates, and the demo's eight rows are not the case to design for when a real month is hundreds. sentAt is a real ISO timestamp now, not the pre-formatted 'Yesterday 10:15' it was, because grouping needs a date to bucket on and parsing the day back out of a display string works right up until the copy changes; grouping is string arithmetic on the ISO value rather than Date parsing, so a 23:30 +04:00 send can't land on the previous day for a reviewer in another timezone, and a fixed demo 'today' keeps screenshots reproducible instead of re-bucketing every midnight. One send sits on 25 Aug so the absolute-date label renders and not just Today/Yesterday. Channel and status filters over every send: recipient, time, body, cost. Failure reasons render inline rather than behind a tooltip — a failed send is the one row type someone is actively hunting for. Two failures in the demo data: a landline that can't take SMS and a hard-bounced mailbox. The footer says 'your most recent sends' rather than claiming to be the whole period, so it can't look like it disagrees with the larger Usage totals.",
      },
      {
        path: "/sales/appointments-list?ref=b-002&view=activity",
      ...
      },
      {
        ...
        note: "DSG-83, the companion to the panel above: Notifications decides whether an event sends, this decides its wording. No send toggle here on purpose — two switches for one fact drift apart. Per-channel tabs, one w-146 card of 7 rows each, keyed on the same ReminderEvent list the Reminders matrix uses so neither surface can disagree about which messages exist. Row text is resolved against sample values and drops the shared 'Hi {{client}}' opener: six of the seven WhatsApp bodies start identically, so a raw excerpt spent its first third saying nothing. Email rows show the subject instead — the line the merchant actually wrote. Open the WhatsApp tab for the shipped default: nothing is switched on yet, so the card header says that once instead of dimming all seven rows, since dimming only reads as 'this one differs' while a sibling is undimmed. SMS is absent deliberately; the ticket names email and WhatsApp.",
      },
      {
        path: "/shell-demo?settings=comms-templates&ce=booking-confirmed:email",
        label: "Communication templates, email editor",
        note: "Opens straight into one editor (&ce=<event>:<channel>). Form left, live preview right and sticky so it stays beside you while you type; stacks below lg. Subject and body are plain text with clickable {{placeholder}} chips — a chip already used in the body tints, so what the message pulls in is visible at a glance. Preview chrome borrows confirmation-email.tsx's visual language rather than the component itself, which composes its own booking content and would show the merchant's copy nowhere. Type a nonsense placeholder like {{discount}}: an unknown token sends as written rather than being silently blanked, because a merchant who typos one needs to see it here and not in a customer's inbox. Save is disabled until something changes; 'Reset to default' only appears once a template is customised. The business name in the preview reads the live business, so the demo rename control changes it.",
      },
      {
        path: "/shell-demo?settings=comms-templates&ce=reminder-24h:whatsapp",
        label: "Communication templates, WhatsApp editor",
  ...
  },
  {
    ...
    screens: [
      ...
      },
      {
        ...
        note: "Email carried over from the sign-in step so it is not retyped. Submitting always reports 'sent', whether or not the address is an HQ account.",
      },
      {
      ...
      },
      {
        ...
        note: "Link older than its window. Offers a fresh request rather than a dead end.",
      },
      {
        path: "/admin/sign-in/reset-password-error?reason=used",
        label: "Reset-password error, already used",
        note: "The link was already spent — a second click on the same email says so instead of failing silently.",
      },
      {
        path: "/admin/sign-in/reset-password-error?reason=invalid",
        label: "Reset-password error, invalid",
        note: "Malformed or unknown token. Same shell as the other two — only the reason line changes.",
      },
      {
        path: "/admin/sign-in-error?reason=not_allowlisted",
        label: "Sign-in error, not allowlisted",
        note: "A valid Google account that is not on the HQ allowlist. Named as a permissions outcome, not a password failure.",
      },
  ...
  },
  {
    ...
    screens: [
      {
        path: "/admin",
        label: "HQ shell (placeholder)",
        note: "What /admin renders today: the HQ shell with its sidebar and topbar around a placeholder body. HQ's real landing is the Partners roster below — listed so the route is not mistaken for a missing dashboard.",
      },
      ...
      },
      {
        ...
        note: "A Partner mid-onboarding: rails off, no rate card, and the actions that need a live account withheld.",
      },
      {
      ...
      },
      {
        ...
        note: "Archived Partner — every tab reads as history: nothing editable, nothing destructive offered.",
      },
      { path: "/admin/audit", label: "Cross-business audit log" },
      {
        path: "/admin/audit?kind=impersonation",
        label: "Audit log, Impersonation only",
        note: "The log filtered to impersonation events — who entered which Partner, when, and on what stated reason.",
      },
    ],
  },
  {
  ...
  {
    lane: "hq",
    title: "Cami HQ, notification control plane",
    description:
      "The HQ half of the notification spec (docs/specs/notifications-sender-id-and-rates.md): per-partner channel grants, Sender ID approval, rate overrides, and the consumption view an invoice is computed from. Answers GNK's points 2 and 3 — 'on HQ side enable/disable email and sms toggle against every merchant' and 'billing tire screen for every merchant where we can see consumption of sms and email with rates'. The partner record carries no notification config until HQ touches it: absent means 'inherits every platform default', because storing a copy of the defaults goes stale the moment they change. Five demo partners cover the useful states — Shampooch (approved Sender ID, global rates), Pawhaus (WhatsApp granted, SMS on a negotiated override), Velvet Paw (onboarding, no Sender ID, SMS not yet granted), Doggos (Sender ID rejected, so SMS is off), Furry Tales (no config at all, proving the inherit path renders).",
    screens: [
      {
        path: "/admin/billing",
        label: "Billing, notification consumption",
      ...
      },
      {
        ...
        label: "HQ settings → Notification rates",
        note: 'The platform defaults every partner inherits, per channel. Try typing a rate: the field is bound to what you type, not to the number — it used to be, and parsing every keystroke back with Number ate the decimal point the instant it was entered ("0." → 0 → "0"), so a decimal rate was untypeable and digits merely accumulated into AED 2266 per message. Validation now guards the string, which is what lets "0." and "0.0" through as intermediate states while still refusing letters, signs and a second point. Above AED 1.00 a warning names the channel and the likely cause — a warning, not a block, since the rate is a commercial decision, but AED 1.00 is eight times the highest real rate (SMS 0.12) so the gap between aggressive and typo is wide. It deliberately stays quiet at 0.24, which is a legitimate doubled SMS segment. Editing confirms on blur rather than per keystroke, matching the per-partner override editor — there is no Save because writes persist as typed, but a rate that changes what every partner is billed shouldn\'t do it in silence. SMS is the one row with a description, because it carries a fact the field can\'t: the rate is per 160-character segment, so a long message bills twice (PRO-989 — the payment link alone is 69 characters). Row padding follows the content, so a row with no description isn\'t 77px of box around 20px of text.',
      },
      {
    ...
    screens: [
      {
        ...
        label: "Access denied",
        note: "Where a permission check lands someone whose role does not carry the scope — the wall HQ routes to rather than rendering a half-empty page.",
      },
      {
      ...
      <header className="mb-10 flex items-start justify-between gap-6">
        <div>
          ...
          <p className="mt-1 max-w-xl text-sm text-muted-foreground">
            Every screen currently routed on this branch, grouped by who uses it. {TOTAL} total.
            Click a row to open the live page in a new tab.
          </p>
        </div>
        <ThemeToggle />
      </header>
